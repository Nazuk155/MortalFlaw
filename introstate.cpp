...
void IntroState::Init()
{
    if( !font )
    {
        font = TTF_OpenFont( BaseFolder "asset/font/RobotoSlab-Bold.ttf", 24 );
    ...
    if( !image )
    {
        image = IMG_LoadTexture( render, BaseFolder "asset/graphic/background.png" );
        if( !image )
    ...
    if( !music )
    {
        music = Mix_LoadMUS( BaseFolder "asset/music/severance.ogg" );
        if( !music )
    ...
    if( !sound )
    {
        sound = Mix_LoadWAV( BaseFolder "asset/sound/pew.wav" );
        if( !sound )
